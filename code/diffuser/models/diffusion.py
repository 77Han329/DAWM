...
class GaussianDiffusion(nn.Module):  #new param: trajectory_dim
    def __init__(self, model, env, horizon, observation_dim, action_dim, reward_dim, trajectory_dim, n_timesteps=1000,
        ...
        # self.transition_dim = observation_dim + reward_dim
        self.trajectory_dim = trajectory_dim
        self.cond_dim = observation_dim + action_dim
        self.model = model
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, returns=None, verbose=True, return_diffusion=False, r_infer=0.5):
        device = self.betas.device

        batch_size = shape[0]
        x = 0.5*torch.randn(shape, device=device)
        x = apply_conditioning(x, cond, self.cond_dim)

        if return_diffusion: diffusion = [x]

        total_timesteps = self.n_timesteps  # K=5
        N = math.ceil(r_infer * total_timesteps)  # N=3
        N_timesteps = torch.linspace(0, total_timesteps - 1, N).long().to(device)  # tensor([0, 2, 4])

        progress = utils.Progress(N) if verbose else utils.Silent()
        for i in reversed(N_timesteps):
    ...
    def conditional_sample(self, cond, returns=None, horizon=None, *args, **kwargs):
        ...
        shape = (batch_size, self.trajectory_dim)
    ...
    def p_losses(self, x_start, cond, t, returns=None):
        ...
        if self.predict_epsilon:
            ...
            noise[:, :(self.observation_dim + self.action_dim)] = 0

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_noisy = apply_conditioning(x_noisy, cond, self.cond_dim)
        ...
        if not self.predict_epsilon:
            x_recon = apply_conditioning(x_recon, cond, self.cond_dim)
...
#     def p_losses(self, x_start, cond, t, returns=None): #x_start: states only?
#         noise = torch.randn_like(x_start)
