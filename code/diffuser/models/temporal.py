...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):  # [(0, (512, 1024)), (1, (128, 512))]
            is_last = ind >= (num_resolutions - 1)
            ...
            self.ups.append(nn.ModuleList([
                ...
                Upsample1d(dim_in, self.env) if not is_last else nn.Identity()
            ]))
        ...
        if self.env[:3] == 'hop':
            conv_layer = nn.Conv1d(dim, transition_dim, 2)
        else:
            conv_layer = nn.Conv1d(dim, transition_dim, 2, 1, 1)
    ...
    def forward(self, x, cond, time, returns=None, use_dropout=True, force_dropout=False):
        '''
            x : [ batch x trajectory ]
            returns : [batch x trajectory]
        '''
        if self.calc_energy:
            x_inp = x
        
        x = x.unsqueeze(1)  # x shape: (b, traj_dim(99 or 150)) -> (b, 1, traj_dim(99)) 

        # x = einops.rearrange(x, 'b h t -> b t h')
        # x shape: (b traj_dim(99))

        t = self.time_mlp(time)  # shape: (b, embed_dim(128))

        action = cond[:, -self.action_dim:]

        a = self.action_mlp(action) # shape: (b, embed_dim(128))

        if self.returns_condition:
            assert returns is not None
            returns_embed = self.returns_mlp(returns)  # shape = (embed_dim)
            if use_dropout:
                mask = self.mask_dist.sample(sample_shape=(returns_embed.size(0), 1)).to(returns_embed.device)
                returns_embed = mask*returns_embed
            if force_dropout:
                returns_embed = 0*returns_embed
            t = torch.cat([t, a, returns_embed], dim=-1)  # shape = (3*embed_dim)
            # print(t.shape)
        ...
        # print('x.shape mid1: ', x.shape)
        x = self.mid_block2(x, t)
        # print('x.shape mid2: ', x.shape)

        # import pdb; pdb.set_trace()

        for resnet, resnet2, upsample in self.ups:
            # print('x shape to concat: ', x.shape)
            # print('h.pop shape: ', h.pop().shape)
            x = torch.cat((x, h.pop()), dim=1)
            x = resnet(x, t)
            x = resnet2(x, t)
            x = upsample(x)

        # print('x before final_conv: ', x.shape)
        ...
        # x = einops.rearrange(x, 'b t -> b h t')
    ...
    def get_pred(self, x, cond, time, returns=None, use_dropout=True, force_dropout=False):
        ...
        '''
        # x = einops.rearrange(x, 'b h t -> b t h')
        x = x.unsqueeze(1) # x shape: (b, traj_dim(99)) -> (b, 1, traj_dim(99)) 

        t = self.time_mlp(time)

        action = cond[:, -self.action_dim:]

        a = self.action_mlp(action)
        ...
        if self.returns_condition:
            ...
            if force_dropout:
                returns_embed = 0*returns_embed
            t = torch.cat([t, a, returns_embed], dim=-1)
