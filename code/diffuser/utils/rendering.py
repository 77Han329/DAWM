...
class MuJoCoRenderer:
    ...
    def renders(self, samples, partial=False, **kwargs):
        ...
        # Code below is used to compress a video into a image
    ...
    def composite(self, savepath, paths, dim=(1024, 256), **kwargs):
        ...
        #TODO: update render_kwargs in kwargs
        images = []
        for path in paths:
            ## [ H x obs_dim ]
            path = atmost_2d(path)
            # img = self.renders(to_np(path), dim=dim, **kwargs)
    ...
    def render_diffusion(self, savepath, diffusion_path, **video_kwargs):
        ...
        for t in reversed(range(n_diffusion_steps)):
            ...
            for states in states_l:
                ...
                # img = self.composite(None, states, dim=(256, 256), partial=True, qvel=True, render_kwargs=render_kwargs)
