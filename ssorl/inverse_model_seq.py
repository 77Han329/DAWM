...
def predict_actions_unlabeled_trajs(
    ...
    print("Predicting actions for unlabeled trajectories")
    ...
    avg_inference_time = total_inference_time / len(new_trajs)
    print(f"Average inference time per trajectory: {avg_inference_time:.6f} seconds")
...
def predict_actions_for_dwm_trajs(
    inverse_models,
    offline_trajs,
    dwm_state_mean,
    dwm_state_std,
    ...
    for traj in new_trajs:
        if num_past_transitions == 0:
            ss = traj["observations"]
            ss = (ss - dwm_state_mean) / dwm_state_std
            ss = torch.from_numpy(ss).to(dtype=torch.float32)

            next_ss = traj["next_observations"]
            next_ss = (next_ss - dwm_state_mean) / dwm_state_std
            next_ss = torch.from_numpy(next_ss).to(dtype=torch.float32)
        ...
        # fill in predicted actions except the first one
        traj["actions"][1:] = mu[1:]
        traj["uncertainty"] = var.mean(axis=1)
